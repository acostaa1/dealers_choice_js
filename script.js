    ...
    const html = poker.map(info=> {
        ...
        <body>
            ...
            <p>Poker is a family of card games in which players wager over which hand is best according to that specific game's rules in ways similar to these rankings. 
               ...
               It was founded in ${info.founded}.
        </body>
        </html>`
    }).join("")
    res.send(html);
...
app.get('/rules', (req, res, next) => {
    const rulesHtml = poker.map(info => {
        return `
        <html>
        <head>
        ...
        <body>
            ...
            only the best five-card combination counts. There are ${info.winningHands} different kinds of poker hands, such as straight flush and four of a kind.- Wiki</p>
    
            You must have ${info.numPlayers} people to play. 
        </body>
        </html>`
    }).join("");
    res.send(rulesHtml);
...
app.get('/hands', (req, res, next) => {
    const handsHtml = poker.map(info => {
        return `<html>
        <head>
        ...
        <body>
            ...
            <p>There are ${info.winningHands} winning hands: </p>
